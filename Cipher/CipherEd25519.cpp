...
namespace
{
  ...
  {
    EDPOINT acc,base,tmp;if(!PointIdentity(acc)||!base.X.CopyFrom(&point.X)||!base.Y.CopyFrom(&point.Y)||!base.Z.CopyFrom(&point.Z)||!base.T.CopyFrom(&point.T))return false;
    int bits=scalar.GetMSB();for(int i=0;i<bits;i++) { if(scalar.GetBit(i)){if(!PointAdd(tmp,acc,base,prime,d))return false;acc.X.Swap(&tmp.X);acc.Y.Swap(&tmp.Y);acc.Z.Swap(&tmp.Z);acc.T.Swap(&tmp.T);} if(!PointDouble(tmp,base,prime))return false;base.X.Swap(&tmp.X);base.Y.Swap(&tmp.Y);base.Z.Swap(&tmp.Z);base.T.Swap(&tmp.T); }
